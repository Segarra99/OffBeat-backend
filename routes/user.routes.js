...
router.post('/profile/:userId/edit', fileUploader.single('profile-picture'), async(req,res)=>{
    try {
        let {userId} = req.params;
        let {username, firstName, lastName, nationality, description, genres} = req.body;
        let currentUser = await User.findById(req.session.currentUser._id);
        let profileUser = await User.findById(userId);
        await User.findByIdAndUpdate(userId, {username, firstName, lastName, nationality, description, genres, img: req.file.path});
    ...
    try {
        ...
        if(permission){
            ...
            req.session.destroy((err)=>{
                if(err){
                    res.status(500).render('/profile/:userId', { errorMessage: err.message});
                    return;
                }
            })
        }
...
router.post('/profile/:userId/review', fileUploader.single('review-picture'), async(req,res)=>{
    try {
